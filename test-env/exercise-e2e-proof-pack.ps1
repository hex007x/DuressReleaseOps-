...
$pricingRegressionRoot = Join-Path $OutputRoot "pricing-regression"
...
$results.Add((Invoke-And-Capture -Name "07-pricing-regression-suite" -Action {
  powershell -NoProfile -ExecutionPolicy Bypass -File $pricingRegressionScript -OutputRoot $pricingRegressionRoot
}))

$results.Add((Invoke-And-Capture -Name "08-known-issue-regression-suite" -Action {
  powershell -NoProfile -ExecutionPolicy Bypass -File $knownIssueRegressionScript -OutputRoot $knownIssueRoot
}))

$results.Add((Invoke-And-Capture -Name "09-commercial-regression-suite" -Action {
  powershell -NoProfile -ExecutionPolicy Bypass -File $commercialRegressionScript -OutputRoot $commercialRoot
}))

$results.Add((Invoke-And-Capture -Name "10-msi-upgrade-metadata-suite" -Action {
...
$lines += "- [Pricing regression summary]($($pricingRegressionRoot -replace '\\','/')/PRICING_REGRESSION_SUMMARY.md)"
...
$lines += "- Pricing foundation, snapshot-backed quote/payment/subscription flows, and admin commercial snapshot visibility"
